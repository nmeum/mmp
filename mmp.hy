...
(defn start-server [addr port songs beets]
  (let [handler  (fn [cmd] (commands.call songs beets cmd))]
...
(defmain [&rest args]
  (let [parser (argparse.ArgumentParser)]
    ...
    (let [args (parser.parse-args)]
      (start-server args.a args.p (SongQueue) (Client args.URL))))
  0)
